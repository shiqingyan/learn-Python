...
def create(cur):
    ...
    except DB_EXC.OperationlError as e:
        drop(cur)
        create(cur)
...
def update(cur):
    ...
    to = rang(1, 5)
    cur.execute(
        "UPDATA users SET projid = %d WHERE projid = %d" % (to, fr))
    return fr, to, getRC(cur)


def delete(cur):
    rm = rand(1, 5)
    cur.execute('DELETE FROM users WHERE projid = %d' % (to, fr))
    return fr ,getRC(cur)
